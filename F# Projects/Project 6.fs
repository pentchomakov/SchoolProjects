...
let rec expand num den radix =
    if (den = 0) then failwith "Cannot divide by 0"
    let remainder = ((num % den) * radix) % den
    let result = (num * radix) / den
    Seq.delay(fun () -> cons result (expand remainder den radix))

//let t = expand 25 7 10;;
...
//let sigma10 = Seq.delay (fun () -> Seq.initInfinite (fun n -> (Term((float)(fact(n)), n))))
//sigma1;;
//let sigma2 = integrateSeries sigma1

// Exponential Series: Sum = 1 + x^1/1! + x^2/2! + x^3/3!...
let rec expSeries = 
    Seq.delay (fun () -> cons (Term (1.0, 0)) (cons (Term(1.0, 1)) (integrateSeries(rest expSeries))))
